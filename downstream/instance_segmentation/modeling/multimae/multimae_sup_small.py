...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        return_token_types = [return_token_types[-1]] * num_out_tokens
    ...
    def forward_features(self,
        ...
        s1_tokens, s2_tokens, dem_tokens, = \
            map(lambda t: rearrange(t, 'b ... d -> b (...) d'),
                (s1_tokens, s2_tokens, dem_tokens))
        # -----------------------mask zorro----------------------------
        tokens = torch.cat((s1_tokens, s2_tokens, dem_tokens), 1)
        ...
        return_tokens, N_H, N_W = self.input_adapters['fusion'](return_tokens)
        # -------------------------Output features-------------------------------------------------------
        # Decode tokens (encoded fusion tokens) for each task using task-specific output adapters
        return_tokens = self.attn_pool(return_tokens, context=tokens)
        return_tokens = return_tokens + self.mlp(self.norm(return_tokens))
        encoder_fusion_tokens = return_tokens
...
class ViTBaseline(MultiMAE):
    ...
    def forward_features(self,
        ...
        for index, blk in enumerate(self.blocks):
            tokens = blk(tokens, None)
        return_tokens = self.attn_pool(return_tokens, context=tokens, attn_mask=None)
        return_tokens = return_tokens + self.mlp(self.norm(return_tokens))
        outs.append(return_tokens)
        outs.append(return_tokens)
        outs.append(return_tokens)
        outs.append(return_tokens)
        return outs, N_H, N_W
