...
from utils import Instances, Boxes, ImageList
...
from utils.criterion_seg import SetCriterion, Criterion, SetCriterion_ins
from utils.matcher_seg import HungarianMatcher
...
class MaskFormer:
    def __init__(self, cfg):
        ...
        self.start_epoch = 0
        self.in_domains = cfg.MultiMAE.in_domains
        self.ignore_idx = 0
    ...
    def build_optimizer(self):
        ...
        if optimizer_type == "SGD":
            ...
            #optimizer = maybe_add_full_model_gradient_clipping(torch.optim.SGD)(
            #    filter(lambda p: p.requires_grad, self.model.parameters()), self.last_lr, momentum=0.9, weight_decay=0.0001)
        elif optimizer_type == "ADAMW":
            optimizer = maybe_add_full_model_gradient_clipping(torch.optim.AdamW)(
                self.model.parameters(), self.last_lr)
            #optimizer = maybe_add_full_model_gradient_clipping(torch.optim.AdamW)(
            #    filter(lambda p: p.requires_grad, self.model.parameters()), self.last_lr)
        else:
    ...
    def train(self, train_sampler, data_loader, eval_loder, n_epochs):
        ...
        for epoch in range(self.start_epoch + 1, n_epochs):
            ...
            if (epoch + 1) % 10 == 0: #evaluator_score > max_score:
    ...
    def train_epoch(self, data_loader, epoch):
        ...
        for i, batch in enumerate(data_loader):
            ...
            inputs = {k: v.to(self.device) for k, v in batch.items() if k in self.in_domains}
            target = batch['label']
            ...
            loss.backward()
            # loss = self.reduce_mean(loss, dist.get_world_size()) # 用于平均所有GPU上的loss
            self.optim.step()
    ...
    @torch.no_grad()
    def evaluate(self, eval_loder, mode=None):
        self.model.eval()
        self.criterion.eval()
        dice_score = []
        conf_mat = metrics.ConfMatrix(self.num_classes + 1)
        ...
        for i, batch in enumerate(eval_loder):
            ...
            if mode == 'eval':
                sem_seg = pred_masks.to('cpu')
                # calculate error metrics
                #print(target.shape, sem_seg.max(1)[1].shape)
                conf_mat.add_batch(target, sem_seg.max(1)[1] + 1)
            else:
                pred_mask = pred_masks[0]
                gt_binary_mask = self._get_binary_mask(target[0])
