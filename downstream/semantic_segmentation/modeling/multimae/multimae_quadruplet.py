...
from .zorro_utils_quadruplet import Block, Block_Fusion, Attention, TokenTypes, LayerNorm, exists, Mlp, AttentionBiLSTM
from .input_adapters import PatchedInputAdapter, FusionInputAdapter, SemSegInputAdapter
...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.return_token_dnw = nn.Parameter(torch.randn(1, 1, dim_tokens))
        # self.attn_dnw = Attention(dim=dim_tokens, dim_head=dim_head, heads=heads)
        ...
        # self.mlp_dnw = Mlp(in_features=dim_tokens, hidden_features=mlp_hidden_dim)
        # task embeddings
    ...
    def forward_features2(self,
        ...
        # fusion_tokens = complete_fusion_tokens[:, torch.cat((s1_idx, s2_idx, dem_idx, dnw_idx), 0), :]
        ...
        for blk, fus in zip(self.blocks, self.fus_blocks):
            ...
            s1_tokens_f[:, s1_idx, :] = tokens[:, sum_idx:sum_idx + len(s1_idx), :]
            sum_idx = sum_idx + len(s1_idx)
            s2_tokens_f = torch.clone(mask_embeddings)
            s2_tokens_f[:, s2_idx, :] = tokens[:, sum_idx:sum_idx + len(s2_idx), :]
            sum_idx = sum_idx + len(s2_idx)
            dem_tokens_f = torch.clone(mask_embeddings)
            dem_tokens_f[:, dem_idx, :] = tokens[:, sum_idx:sum_idx + len(dem_idx), :]
            sum_idx = sum_idx + len(dem_idx)
            dnw_tokens_f = torch.clone(mask_embeddings)
            dnw_tokens_f[:, dnw_idx, :] = tokens[:, sum_idx:sum_idx + len(dnw_idx), :]
            ...
            all_tokens = torch.cat((s1_tokens_f.unsqueeze(2), s2_tokens_f.unsqueeze(2), dem_tokens_f.unsqueeze(2),
                                    dnw_tokens_f.unsqueeze(2), fus_tokens_f.unsqueeze(2)), dim=2)
            # all_tokens = rearrange(all_tokens, 'b n m d -> (b n) m d')
            # fusion_tokens = self.attn_pool_modalities(all_tokens)
            # fusion_tokens = rearrange(fusion_tokens[:, -1, :].squeeze(1), '(b n) d -> b n d', b=B)
...
class ViTBaseline(MultiMAE):
    ...
    def forward_features(self,
        ...
        if one_mod in x:
            if len(x[one_mod].shape) > 3:
                B, C, H, W = x[one_mod].shape
            else:
                B, H, W = x[one_mod].shape
        else:
            B, C, H, W = list(x.values())[0].shape  # TODO: Deal with case where not all have same shape
        ...
        if 'dem' in self.incomplete_domains:
            ...
            all_idx['dem'] = dem_idx
        if 'dnw' in self.incomplete_domains:
            dnw_idx = ((task_masks['dnw'][0, :] == 0).nonzero(as_tuple=True)[0])
            dnw_tokens = input_task_tokens['dnw'][:, dnw_idx, :]
            dnw_tokens = rearrange(dnw_tokens, 'b ... d -> b (...) d')
            all_tokens['dnw'] = dnw_tokens
            all_idx['dnw'] = dnw_idx
        ...
        type_dict = {'s1': TokenTypes.S1.value, 's2': TokenTypes.S2.value, 'dem': TokenTypes.DEM.value,
                     'dnw': TokenTypes.DNW.value, 'fusion': TokenTypes.FUSION.value}
        token_type_list = []
    ...
    def _freeze_stages(self):
        ...
        if self.frozen_stages >= 0:
            ...
            for param in self.attn_pool.parameters():
                param.requires_grad = False
            for param in self.attn_lstm.parameters():
                param.requires_grad = False
...
def ViTMAE(args, *argss, **kwargs):
    ...
    model = ViTBaseline(
        ...
        return_token_types=(TokenTypes.S1, TokenTypes.S2, TokenTypes.DEM, TokenTypes.DNW, TokenTypes.FUSION),
