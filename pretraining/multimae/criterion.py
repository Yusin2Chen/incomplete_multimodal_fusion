...
def dino_loss_func(student_output, teacher_output, teacher_temp=0.04, student_temp=0.1):
    # fusion = student, each modality = teacher
    student_output = F.normalize(student_output, dim=1)
    teacher_output = F.normalize(teacher_output, dim=1)
    student_out = F.log_softmax(student_output / student_temp, dim=-1)
    teacher_out = F.softmax(teacher_output / teacher_temp, dim=-1).detach()
