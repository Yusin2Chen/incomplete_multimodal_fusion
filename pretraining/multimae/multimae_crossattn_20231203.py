...
from .zorro_utils import Block, Attention, CrossAttention, TokenTypes, LayerNorm, exists, Mlp, AttentionBiLSTM
...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        # return tokens for each domain in fusion tokens ---------------------------------------------------------------
        self.return_token_s1 = nn.Parameter(torch.randn(1, 1, dim_tokens))
        self.attn_s1 = Attention(dim=dim_tokens, dim_head=dim_head, heads=heads)
        self.return_token_s2 = nn.Parameter(torch.randn(1, 1, dim_tokens))
        self.attn_s2 = Attention(dim=dim_tokens, dim_head=dim_head, heads=heads)
        self.return_token_dem = nn.Parameter(torch.randn(1, 1, dim_tokens))
        self.attn_dem = Attention(dim=dim_tokens, dim_head=dim_head, heads=heads)

        mlp_hidden_dim = int(dim_tokens * 4.0)
        self.mlp = Mlp(in_features=dim_tokens, hidden_features=mlp_hidden_dim)
        self.mlp_modalities = Mlp(in_features=dim_tokens, hidden_features=mlp_hidden_dim)
        self.mlp_s1 = Mlp(in_features=dim_tokens, hidden_features=mlp_hidden_dim)
        self.mlp_s2 = Mlp(in_features=dim_tokens, hidden_features=mlp_hidden_dim)
        self.mlp_dem = Mlp(in_features=dim_tokens, hidden_features=mlp_hidden_dim)

        # self.attn_lstm = AttentionBiLSTM(dim_tokens)
        ...
        # task embeddings
        self.attn_pool_modalities = CrossAttention(dim_q=dim_tokens, dim=dim_tokens, num_heads=heads)
        # learnt tokens
    ...
    def forward(self,
        ...
        complete_fusion_tokens = repeat(self.fusion_tokens, '() n d -> b n d', b=B)
        complete_fusion_tokens = self.input_adapters['fusion'](complete_fusion_tokens)
        ...
        fusion_tokens = complete_fusion_tokens[:, torch.cat((s1_idx, s2_idx, dem_idx), 0), :]
        ...
        for blk in self.blocks:
            ...
            query_emb = rearrange(tokens[:, num_encoded_tokens:, :], 'b n d -> (b n) d').unsqueeze(1)
            mask_embeddings = repeat(self.mask_embedding, '() n d -> b n d', b=B)

            s1_tokens_f = torch.clone(mask_embeddings).scatter(1, s1_idx.unsqueeze(0).unsqueeze(2).repeat(B, 1, 768), tokens[:, :len(s1_idx), :])
            s2_tokens_f = torch.clone(mask_embeddings).scatter(1, s2_idx.unsqueeze(0).unsqueeze(2).repeat(B, 1, 768), tokens[:, len(s1_idx):len(s1_idx) + len(s2_idx), :])
            dem_tokens_f = torch.clone(mask_embeddings).scatter(1, dem_idx.unsqueeze(0).unsqueeze(2).repeat(B, 1, 768), tokens[:, num_encoded_tokens - len(dem_idx):num_encoded_tokens, :])
            # s1_tokens_f = (task_masks['s1'].unsqueeze(-1) == 1) * mask_embeddings + (task_masks['s1'].unsqueeze(-1) == 0) * input_task_tokens['s1']
            # s2_tokens_f = (task_masks['s2'].unsqueeze(-1) == 1) * mask_embeddings + (task_masks['s2'].unsqueeze(-1) == 0) * input_task_tokens['s2']
            # dem_tokens_f = (task_masks['dem'].unsqueeze(-1) == 1) * mask_embeddings + (task_masks['dem'].unsqueeze(-1) == 0) * input_task_tokens['dem']

            all_tokens = torch.cat((s1_tokens_f.unsqueeze(2), s2_tokens_f.unsqueeze(2), dem_tokens_f.unsqueeze(2)), dim=2)
            all_tokens = rearrange(all_tokens, 'b n m d -> (b n) m d')
            fusion_tokens = self.attn_pool_modalities(query_emb, context=all_tokens)
            fusion_tokens = rearrange(fusion_tokens.squeeze(1), '(b n) d -> b n d', b=B)
            fusion_tokens = tokens[:, num_encoded_tokens:, :] + self.mlp_modalities(self.norm(fusion_tokens))
        ...
        encoder_fusion_tokens = tokens[:, num_encoded_tokens:, :]  # for outadapters 乱序
        # restore idx
        selected_idx = torch.cat((s1_idx, s2_idx, dem_idx), 0)
        # print(selected_idx.shape[0])
        for i in range(selected_idx.shape[0]):
            complete_fusion_tokens[:, selected_idx[i], :] = encoder_fusion_tokens[:, i, :]

        # downstream tasks
        preds = {
            domain: self.output_adapters[domain](
                encoder_tokens=complete_fusion_tokens,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            for domain in fp32_output_adapters:
                ...
                preds[domain] = self.output_adapters[domain](
                    encoder_tokens=complete_fusion_tokens.float(),
                    input_info=input_info,
                    ids_keep=ids_keep,
                    ids_restore=ids_restore,
                )
        ...
        return_token_s1 = repeat(self.return_token_s1, '() n d -> b n d', b=batch)
        tokens_s1 = encoder_fusion_tokens[:, s1_idx, :]
        return_token_s1 = self.attn_s1(return_token_s1, context=tokens_s1, attn_mask=None)
        return_token_s1 = return_token_s1 + self.mlp_s1(self.norm(return_token_s1))

        return_token_s2 = repeat(self.return_token_s2, '() n d -> b n d', b=batch)
        tokens_s2 = encoder_fusion_tokens[:, s2_idx, :]
        return_token_s2 = self.attn_s2(return_token_s2, context=tokens_s2, attn_mask=None)
        return_token_s2 = return_token_s2 + self.mlp_s2(self.norm(return_token_s2))

        return_token_dem = repeat(self.return_token_dem, '() n d -> b n d', b=batch)
        tokens_dem = encoder_fusion_tokens[:, dem_idx, :]
        return_token_dem = self.attn_dem(return_token_dem, context=tokens_dem, attn_mask=None)
        return_token_dem = return_token_dem + self.mlp_dem(self.norm(return_token_dem))

        return preds, task_masks, return_tokens, ori_tokens, encoder_fusion_tokens, return_token_s1, return_token_s2, return_token_dem
