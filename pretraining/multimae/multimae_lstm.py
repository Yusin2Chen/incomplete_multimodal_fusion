...
from .zorro_utils import Block, Attention, TokenTypes, LayerNorm, exists, Mlp, AttentionBiLSTM
...
class MultiMAE(nn.Module):
    ...
    def forward(self,
        ...
        # merge all modalities tokens into fusion token
        all_tokens = torch.cat((tokens[:, :num_encoded_tokens, :].unsqueeze(2),
                                tokens[:, num_encoded_tokens:, :].unsqueeze(2)), dim=2)
        all_tokens = rearrange(all_tokens, 'b n m d -> (b n) m d')
        fusion_tokens = self.attn_lstm(all_tokens, mask=None)
        fusion_tokens = rearrange(fusion_tokens, '(b n) d -> b n d', b=B, n=num_encoded_tokens)
        ...
        return_token_s1 = self.attn_pool(return_token_s1, context=tokens_s1, attn_mask=None)
        return_token_s1 = return_token_s1 + self.mlp(self.norm(return_token_s1))

        return_token_s2 = repeat(self.return_token_s2, '() n d -> b n d', b=batch)
        tokens_s2 = encoder_fusion_tokens[:, s2_idx, :]
        return_token_s2 = self.attn_pool(return_token_s2, context=tokens_s2, attn_mask=None)
        return_token_s2 = return_token_s2 + self.mlp(self.norm(return_token_s2))

        return_token_dem = repeat(self.return_token_dem, '() n d -> b n d', b=batch)
        tokens_dem = encoder_fusion_tokens[:, dem_idx, :]
        return_token_dem = self.attn_pool(return_token_dem, context=tokens_dem, attn_mask=None)
        return_token_dem = return_token_dem + self.mlp(self.norm(return_token_dem))
