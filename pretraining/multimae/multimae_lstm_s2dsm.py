...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        assert num_fusion_tokens == input_adapters['s2'].num_patches
    ...
    def forward(self,
        ...
        x = {'s2': x} if isinstance(x, torch.Tensor) else x
        batch, device = x['s2'].shape[0], x['s2'].device
        ...
        fusion_tokens = complete_fusion_tokens[:, torch.cat((s2_idx, dem_idx), 0), :]
        ...
        s2_tokens, dem_tokens, fusion_tokens = \
        ...
        # -----------------------mask zorro----------------------------
        tokens = torch.cat((s2_tokens, dem_tokens, fusion_tokens), 1)
        ...
        # restore idx
        selected_idx = torch.cat((s2_idx, dem_idx), 0)
