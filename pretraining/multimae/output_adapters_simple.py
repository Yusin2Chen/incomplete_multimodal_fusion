...
class SpatialOutputAdapter(nn.Module):
    ...
    def no_weight_decay(self):
        return {'pos_emb', 'task_embeddings'}
    ...
    def forward(self,
        ...
        if self.task_embeddings is not None and self.task in self.task_embeddings:
            task_emb = repeat(self.task_embeddings[self.task], '() () d -> b n d', b=B, n=N_H * N_W)
            context_tokens = context_tokens + task_emb

        # Optional transformer layers if depth > 0
        x = self.decoder_transformer(context_tokens)
