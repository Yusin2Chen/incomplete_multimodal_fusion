...
from multimae.criterion import MaskedL1Loss, MaskedMSELoss, vicreg, HardNegtive_loss, DINOLoss, byol_loss_func, dino_loss_func
...
from utils.multimodal_dfc2023 import DFC2023
...
def get_args():
    ...
    parser.add_argument('--batch_size', default=60, type=int, help='Batch size per GPU (default: %(default)s)')
    parser.add_argument('--epochs', default=800, type=int, help='Number of epochs (default: %(default)s)')
    ...
    parser.add_argument('--standardize_depth', default=False, action='store_true')
    parser.add_argument('--no_standardize_depth', default=True, action='store_false', dest='standardize_depth')
    parser.set_defaults(standardize_depth=False)
    parser.add_argument('--extra_norm_pix_loss', default=False, action='store_true')
    parser.add_argument('--no_extra_norm_pix_loss', default=True, action='store_false', dest='extra_norm_pix_loss')
    parser.set_defaults(extra_norm_pix_loss=False)
    ...
    parser.add_argument('--num_encoded_tokens', default=384, type=int, help='256 Number of tokens to randomly choose for encoder (default: %(default)s)')
    ...
    parser.add_argument('--loss_on_unmasked', default=False, action='store_true', help='Set to True/False to enable/disable computing the loss on non-masked tokens')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/workspace/DFC2023/crop/images', type=str, help='dataset path')
    # Misc.
    parser.add_argument('--output_dir', default='./save_attention', help='Path where to save, empty for no saving')
...
def get_model(args):
    ...
    if args.extra_norm_pix_loss:
        output_adapters['norm_s1'] = DOMAIN_CONF['s1']['output_adapter'](
            stride_level=DOMAIN_CONF['s1']['stride_level'],
            patch_size_full=args.patch_size,
            dim_tokens=args.decoder_dim,
            depth=args.decoder_depth,
            num_heads=args.decoder_num_heads,
    ...
    model = pretrain_multimae_tiny(
        ...
        num_fusion_tokens=256,  # number of fusion tokens 256
...
def main(args):
    ...
    contra_loss = HardNegtive_loss()
    #contra_loss = DINOLoss(out_dim=384)
    #-------------------------------------------------------------------------------
    ...
    # Get dataset
    dataset_train = DFC2023(args.data_path, use_rgb=True, use_sar=True, use_dsm=True, unlabeled=True,
                            transform=False, crop_size=args.input_size)
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, tasks_loss_fn: Dict[str, torch.nn.Module],
                    ...
                    contra_loss: torch.nn.Module = None):
    model.train()
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        tasks_dict = {
            ...
            for task, tensor in x.items() if task in in_domains
        }

        # Truncated depth standardization
        if standardize_depth and 'dem' in tasks_dict:
            # Flatten depth and remove bottom and top 10% of values
            trunc_depth = torch.sort(rearrange(tasks_dict['dem'], 'b c h w -> b (c h w)'), dim=1)[0]
            trunc_depth = trunc_depth[:, int(0.1 * trunc_depth.shape[1]): int(0.9 * trunc_depth.shape[1])]
            tasks_dict['dem'] = (tasks_dict['dem'] - trunc_depth.mean(dim=1)[:, None, None, None]) / torch.sqrt(
        ...
        with torch.cuda.amp.autocast():
            preds, masks, pooled_tokens, ori_tokens, fus_tokens, s1_tokens, s2_tokens, dem_tokens = model(
                input_dict,
            ...
            if extra_norm_pix_loss:
                tasks_dict['norm_s1'] = tasks_dict['s1']
                masks['norm_s1'] = masks.get('s1', None)
            ...
            # --------------------------contrastive loss-------------------------
            s1_feat, s2_feat, dsm_feat, fusion_feat = torch.chunk(pooled_tokens, 4, dim=1)
            s1_feat, s2_feat, dsm_feat, fusion_feat = s1_feat.squeeze(), s2_feat.squeeze(), dsm_feat.squeeze(), fusion_feat.squeeze()
            s1_tokens, s2_tokens, dem_tokens = s1_tokens.squeeze(), s2_tokens.squeeze(), dem_tokens.squeeze()
            # loss_contra = vicreg(s1_feat, fusion_feat) + vicreg(s2_feat, fusion_feat) + vicreg(dsm_feat, fusion_feat)
            loss_contra = dino_loss_func(s1_tokens, s1_feat) + dino_loss_func(s2_tokens, s2_feat) + dino_loss_func(dem_tokens, dsm_feat)
            # local features
            #ori_tokens = rearrange(ori_tokens, 'b c d -> b (c d)')
            #fus_tokens = rearrange(fus_tokens, 'b c d -> b (c d)')
            #loss_align = byol_loss_func(ori_tokens, fus_tokens)
            # -------------------------------------------------------------------------
            weighted_task_losses = loss_balancer(task_losses)
            loss = sum(weighted_task_losses.values()) + 0.3 * loss_contra
    ...
    print('Dino similarity loss', loss_contra.item())
