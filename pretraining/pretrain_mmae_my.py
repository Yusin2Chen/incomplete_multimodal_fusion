...
from multimae.criterion import MaskedL1Loss, MaskedMSELoss, MaskedCrossEntropyLoss, \
    vicreg, HardNegtive_loss, DINOLoss, byol_loss_func
...
from utils.multimodal_quadruplet import MyDataset
...
def get_args():
    ...
    parser.add_argument('--batch_size', default=20, type=int, help='Batch size per GPU (default: %(default)s)')
    parser.add_argument('--epochs', default=1600, type=int, help='Number of epochs (default: %(default)s)')
    parser.add_argument('--save_ckpt_freq', default=5, type=int, help='Checkpoint saving frequency in epochs (default: %(default)s)')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/workspace/s1s2dsm_patch', type=str, help='dataset path')
    # Misc.
...
def main(args):
    ...
    #s1 = torch.randn(2, 2, 32, 32)  # (batch, channels, height, width)
    #s2 = torch.randn(2, 13, 32, 32)  # (batch, channels, height, width)
    #dem = torch.randn(2, 1, 32, 32)  # (batch, channels, height, width)
    #input_dict = {'s1': s1, 's2': s2, 'dem': dem}
    #preds, masks = model(input_dict, num_encoded_tokens=6, alphas=1.0)
    #print(preds['s1'].shape, masks)
    #-------------------------------------------------------------------------------
    ...
    # Get dataset
    dataset_train = MyDataset(args.data_path, use_s1=True, use_s2=True, use_dem=True, use_dnw=True,
                              unlabeled=True, transform=True, crop_size=args.input_size)
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, tasks_loss_fn: Dict[str, torch.nn.Module],
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            preds, masks, pooled_tokens, ori_tokens, fus_tokens = model(
                input_dict,
            ...
            #loss_contra = contra_loss(s1_feat, fusion_feat) + contra_loss(s2_feat, fusion_feat) + contra_loss(dsm_feat, fusion_feat)
            # local features
            ...
            loss = sum(weighted_task_losses.values()) #+ 1 * loss_align
