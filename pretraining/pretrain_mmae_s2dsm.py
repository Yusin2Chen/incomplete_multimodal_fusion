...
import utils
from multimae.multimae_lstm_s2dsm import pretrain_multimae_base, pretrain_multimae_tiny
from multimae.zorro_utils import TokenTypes as T
from multimae.criterion import MaskedL1Loss, MaskedMSELoss, vicreg, HardNegtive_loss, DINOLoss, byol_loss_func
...
def get_args():
    ...
    parser.add_argument('--batch_size', default=100, type=int, help='Batch size per GPU (default: %(default)s)')
    ...
    # Task parameters
    parser.add_argument('--in_domains', default='s2-dem', type=str, help='Input domain names, separated by hyphen (default: %(default)s)')
    parser.add_argument('--out_domains', default='s2-dem', type=str, help='Output domain names, separated by hyphen (default: %(default)s)')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/workspace/DFC2023/track2/images', type=str, help='dataset path')
    # Misc.
...
def get_model(args):
    ...
    model = pretrain_multimae_tiny(
        ...
        return_token_types=(T.S2, T.DEM, T.FUSION),
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, tasks_loss_fn: Dict[str, torch.nn.Module],
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            # --------------------------contrastive loss-------------------------
            s2_feat, dsm_feat, fusion_feat = torch.chunk(pooled_tokens, 3, dim=1)
            s2_feat, dsm_feat, fusion_feat = s2_feat.squeeze(), dsm_feat.squeeze(), fusion_feat.squeeze()
            # loss_contra = vicreg(s1_feat, fusion_feat) + vicreg(s2_feat, fusion_feat) + vicreg(dsm_feat, fusion_feat)
            loss_contra = contra_loss(s2_feat, dsm_feat) + contra_loss(s2_feat, fusion_feat) + contra_loss(dsm_feat, fusion_feat)
            # local features
            ...
            loss = sum(weighted_task_losses.values()) + 1 * loss_contra
